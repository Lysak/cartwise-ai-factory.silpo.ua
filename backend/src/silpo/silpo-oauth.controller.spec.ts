...
import { sessionCookieName, sessionCookieOptions, type SessionService } from '../auth/session.service';
...
describe('SilpoOauthController discovery', () => {
  ...
  it('sets the opaque session cookie and clears the connect cookie after a successful callback', async () => {
    ...
    expect(response.cookie).toHaveBeenCalledWith(sessionCookieName(), 'session-1', sessionCookieOptions());
  ...
  it('falls through to legacy Telegram completion for a valid legacy state with a stale browser cookie', async () => {
    ...
    expect(response.cookie).toHaveBeenCalledWith(sessionCookieName(), 'telegram-session', sessionCookieOptions());
    expect(response.redirect).toHaveBeenCalledWith(`${origin}/?silpo=connected`);
  });

  it('redirects to a failure screen instead of leaking a raw 401 body when the Telegram state was already consumed or expired', async () => {
    oauth.completeAuthorization.mockRejectedValueOnce(new UnauthorizedException('Invalid or expired authorization state'));
    const controller = subject();
    const response = httpResponse();
