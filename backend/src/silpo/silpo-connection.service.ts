...
export class SilpoConnectionService {
  ...
  async withTrackingReadAccess<T>(operation: (accessToken: string) => Promise<T>): Promise<T> {
    const access = await this.trackingAccessFor();
    try {
      return await operation(access.token);
    } catch (error) {
      if (!isAuthorizationRejection(error)) throw error;
  ...
  private async trackingAccessFor(): Promise<{ id: string; token: string }> {
    const result = await this.pool.query<Connection>(
      'SELECT id, "userId", "accessTokenEncrypted", "refreshTokenEncrypted", "accessTokenExpiresAt", status FROM "SilpoConnection" WHERE "isTrackingService" = TRUE AND status = \'active\''
    );
    const connection = result.rows[0];
    if (!connection) throw new ServiceUnavailableException('Silpo tracking connection unavailable');
