...
export class SilpoOauthService {
  ...
  async callSilpoTool(accessToken: string, name: string, args: Record<string, unknown>, id: string): Promise<Record<string, unknown>> {
    ...
    if (!response.ok) {
      if (response.status === 401) throw new UnauthorizedException('Silpo MCP authorization required');
      if (response.status === 429) throw new HttpException('Silpo MCP rate limited', 429);
  ...
  async listMcpTools(accessToken: string): Promise<SanitizedMcpTool[]> {
    return this.fetchMcpCatalog(accessToken);
  }
