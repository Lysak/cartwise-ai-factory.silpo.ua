...
describe('SilpoConnectionService', () => {
  ...
  it('reads through the marked active tracking service connection without a user ID', async () => {
    const { service, pool } = subject();
    const row = connection(service, {
      accessTokenExpiresAt: new Date(Date.now() + 60 * 60_000),
      isTrackingService: true
    });
    pool.query.mockResolvedValue({ rows: [row] });

    await expect(service.withTrackingReadAccess(async (token) => token)).resolves.toBe('old-access');

    expect(pool.query).toHaveBeenCalledWith(
      expect.stringMatching(/WHERE "isTrackingService" = TRUE.*status = 'active'/s)
    );
    expect(pool.query.mock.calls[0]).toHaveLength(1);
  });

  it('does not fall back to an unmarked customer connection for tracking', async () => {
    const { service, pool } = subject();
    pool.query.mockResolvedValue({ rows: [] });

    await expect(service.withTrackingReadAccess(async (token) => token))
  ...
  it('refreshes and retries tracking reads after authorization rejection', async () => {
    const { service, oauth, pool, tx } = subject();
    ...
    await expect(service.withTrackingReadAccess(operation)).resolves.toBe('ok');
  ...
  it('refreshes and retries when callSilpoTool receives HTTP 401', async () => {
    const { service, oauth, refreshToken, pool, tx } = subjectWithRealOauth();
    const row = connection(service, { accessTokenExpiresAt: new Date(Date.now() + 60 * 60_000) });
    pool.query.mockResolvedValue({ rows: [row] });
    tx.query.mockImplementation(locking(row));
    refreshToken.mockResolvedValue(tokenSet('new-access', 'new-refresh'));
    global.fetch = jest.fn()
      .mockResolvedValueOnce(mcpResponse({}, 401))
      .mockResolvedValueOnce(mcpResponse({ result: { structuredContent: { items: [] } } }));

    await expect(service.withReadAccess('user-1', (token) =>
      oauth.callSilpoTool(token, 'silpo_list_branches', {}, 'id')
  ...
  it('maps callSilpoTool HTTP 401 followed by refresh rejection to reauth_required', async () => {
    const { service, oauth, refreshToken, pool, tx } = subjectWithRealOauth();
    const row = connection(service, { accessTokenExpiresAt: new Date(Date.now() + 60 * 60_000) });
    pool.query.mockResolvedValue({ rows: [row] });
    tx.query.mockImplementation(locking(row));
    refreshToken.mockRejectedValue(new SilpoRefreshRejectedError());
    global.fetch = jest.fn().mockResolvedValue(mcpResponse({}, 401));

    const error = await service.withReadAccess('user-1', (token) =>
      oauth.callSilpoTool(token, 'silpo_list_branches', {}, 'id')
    ).catch((cause) => cause);

    expect(error).toBeInstanceOf(SilpoReauthRequiredException);
    expect(error.getStatus()).toBe(409);
    expect(tx.query).toHaveBeenCalledWith(expect.stringContaining("SET status = 'reauth_required'"), ['connection-1']);
  });
...
function subjectWithRealOauth() {
  const oauth = new SilpoOauthService({} as SessionService);
  const refreshToken = jest.fn();
  Object.assign(oauth, { refreshToken });
  const service = new SilpoConnectionService(oauth);
  const tx = transaction();
  const pool = { query: jest.fn(), connect: jest.fn().mockResolvedValue(tx) };
  Object.assign(service as object, { pool });
  return { service, oauth, refreshToken, pool, tx };
}
...
const mcpResponse = (body: Row, status = 200) => ({
