...
import { sessionCookieName, type SessionService } from './session.service';
...
describe('AuthController', () => {
  ...
  it('creates a local session for an existing verified Telegram user', async () => {
    pool.query
      ...
      .mockResolvedValueOnce({ rows: [{ status: 'active', silpoExternalId: 'provider-id', accessToken: 'raw-token' }] })
      .mockResolvedValueOnce({ rows: [{ preferredSilpoBranchId: 'branch-1', telegramUserId: telegram.telegramUserId }] });
    const res = response();

    const result = await controller.bootstrap('validated-init-data', { ip: '127.0.0.1', cookies: {} }, res as never);
    expect(result).toEqual({
      status: 'authenticated', csrfToken: 'csrf-token', silpoStatus: 'active', preferredSilpoBranchId: 'branch-1', telegramConnected: true
    });

    expect(Object.keys(result).sort()).toEqual(['csrfToken', 'preferredSilpoBranchId', 'silpoStatus', 'status', 'telegramConnected']);
  ...
  it('renews a matching session with its current Silpo status', async () => {
    ...
    pool.query
      .mockResolvedValueOnce({ rows: [{ status: 'active', silpoExternalId: 'provider-id', accessToken: 'raw-token' }] })
      .mockResolvedValueOnce({ rows: [{ preferredSilpoBranchId: 'branch-1', telegramUserId: null }] });

    const result = await controller.bootstrap('validated-init-data', { ip: '127.0.0.1', cookies: { [sessionCookieName()]: 'session-id' } }, response() as never);
    expect(result).toEqual({
      status: 'authenticated', csrfToken: 'csrf-token', silpoStatus: 'active', preferredSilpoBranchId: 'branch-1', telegramConnected: false
    });

    expect(pool.query).toHaveBeenCalledWith('SELECT status FROM "SilpoConnection" WHERE "userId" = $1', ['user-1']);
    expect(pool.query).toHaveBeenCalledWith('SELECT "preferredSilpoBranchId", "telegramUserId" FROM "User" WHERE id = $1', ['user-1']);
    expect(Object.keys(result).sort()).toEqual(['csrfToken', 'preferredSilpoBranchId', 'silpoStatus', 'status', 'telegramConnected']);
    expect(JSON.stringify(result)).not.toContain('provider-id');
    expect(JSON.stringify(result)).not.toContain('raw-token');
  });
  ...
  it('returns reauth_required for a matching session with a reauthentication-required Silpo connection', async () => {
    ...
    await expect(controller.bootstrap('validated-init-data', { ip: '127.0.0.1', cookies: { [sessionCookieName()]: 'session-id' } }, response() as never)).resolves.toEqual({
      status: 'authenticated', csrfToken: 'csrf-token', silpoStatus: 'reauth_required', preferredSilpoBranchId: null, telegramConnected: false
    });
  ...
  it('returns the current CSRF token and connection status for a session', async () => {
    ...
    await expect(controller.session({ cookies: { [sessionCookieName()]: 'session-id' }, user: { id: 'user-1' } })).resolves.toEqual({
      status: 'authenticated', csrfToken: 'csrf-token', silpoStatus: 'reauth_required', preferredSilpoBranchId: null, telegramConnected: false
    });
  });
  ...
  it('returns no provider identity or token material from the session status endpoint', async () => {
    ...
    pool.query.mockResolvedValue({ rows: [{ status: 'active', preferredSilpoBranchId: null, telegramUserId: null, silpoExternalId: 'provider-id', accessToken: 'raw-token' }] });

    const result = await controller.session({ cookies: { [sessionCookieName()]: 'session-id' }, user: { id: 'user-1' } });

    expect(result).toEqual({ status: 'authenticated', csrfToken: 'csrf-token', silpoStatus: 'active', preferredSilpoBranchId: null, telegramConnected: false });
    expect(Object.keys(result).sort()).toEqual(['csrfToken', 'preferredSilpoBranchId', 'silpoStatus', 'status', 'telegramConnected']);
    expect(JSON.stringify(result)).not.toContain('provider-id');
    expect(JSON.stringify(result)).not.toContain('raw-token');
  });
  ...
  it('destroys the session and clears its cookie on logout', async () => {
    ...
    await expect(controller.logout({ cookies: { [sessionCookieName()]: 'session-id' } }, res as never)).resolves.toEqual({ status: 'logged_out' });
